...
const ItemCard = async ({item}: Props) => {
  ...
  return (
    <div className='shadow-md rounded-md border-[1px] border-slate-200 p-2 flex flex-col'>
      <img src={item?.img} style={{aspectRatio:"4/3", width:"100%", objectFit:"contain"}} alt={item.name} /> 
      <div className="p-2 flex flex-col flex-1">
        <Link href={`/items/${item?.id}`} className='flex justify-between items-center gap-4 mb-2' title={item.name}>
        ...
        <div className='flex flex-col justify-between flex-1 gap-2'>
          <Link href={`/items/${item?.id}`} className=' line-clamp-2 ' >{item.description}</Link>
